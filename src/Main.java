import concepts.schedulers.OutputDto;
import concepts.schedulers.ProblemSolver;
...
public class Main {
    ...
    private static final String outputFileName = "output-with-column-size-and-iterations-4-procs.csv";
...
//
        ...
        for(String problem: problemSet) {
            ...
            System.out.println("Solved Problem: " +
                    ...
                    " ms and found the best solution: " +
                    best.cMax );
            ...
            outputData.add(new String[]{
                    ...
                    executionTimeMilliseconds + "",
                    best.cMax + "",
                    //Optional Inputs, comment out to exclude
                    best.columnCounts.get(best.columnCounts.size() - 1) + "", //Final number of iterations
                    best.columnCounts.size() + "", //Final number of columns
                    best.columnCounts.stream().map(String::valueOf).collect(Collectors.joining("=>")), //Columns generated each iteration
                    best.improvements.stream().map(String::valueOf).collect(Collectors.joining("=>")), //Improvement of each iteration
            });
        }
    ...
    public static void createCsvFile(List<String[]> dataLines) throws IOException {
        File csvOutputFile = new File("problem-output/" + outputFileName);
